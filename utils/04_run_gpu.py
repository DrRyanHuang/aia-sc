...
import time
import argparse
from check_fixing import check_fixing_cy
...
def calc_loss(nn, label, last_instance, instance, b, alphai, ci, GAMMA, device):
    left = nn([last_instance, b]).to(device)
    item1 = (nn([instance, b - alphai]) + ci).to(device)
    item0 = nn([instance, b]).to(device)
    if label == 3:
        loss = (left - GAMMA * min(item1, item0)) * (left - GAMMA * min(item1, item0))
    elif label == 2:
        loss = (left - GAMMA * item1) * (left - GAMMA * item1) + 100 * F.relu(item1 - item0)
    return loss
...
while t < T:
    ...
    for i in range(ncol):
        ...
        if label:
            ...
            current_datapool.append(
                [2, last_instance, instance, b_tmp, alphai, ci])
            b_tmp = b_tmp - alphai
        else:
            instance[:, i] = 0
            current_datapool.append(
                [3, last_instance, instance, b_tmp, alphai, ci])
    ...
    for i in range(len(training_batch)):
        ...
        dp_loss += calc_loss(nn, label, last_instance, instance, b, alphai, ci, GAMMA, device)
