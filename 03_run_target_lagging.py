...
def calc_loss(nn, nn_target, label, last_instance, instance, b, alphai, ci):
    left = nn([last_instance, b])
    item1 = (nn_target([instance, b - alphai]) + ci).detach()
    item0 = (nn_target([instance, b])).detach()
    if label == 3:
...
while t < T:
    ...
    # TODO: TDerror & importance sampling?
    # maintain a datapool
